...
ip addr add 2.2.2.101/32 dev $VRF_NAME-lo

#accesss to RDS
ip link set eth1 master $VRF_NAME
...
for IP in `cat /root/webinar.txt`; do

    id=`echo $IP| awk -F'.' '{printf "%s-%s",$3,$4}'`
    ...
    sudo ip link add $interface type geneve vni $vni remote $IP
